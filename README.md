...
| PermissionRequest 层（第二层） | hooks.json 不设 matcher（匹配所有工具）：客户端任何工具的弹窗请求——子智能体创建（Agent/Task）、名单外工具（Write/Edit）、MCP/扩展工具——一律强制送审，allow/deny 输出决策；只有模型不可用兜底 ask、plan 模式、输入读不懂、无任务内容的子代理请求或第一层刚裁定的人工路径退避到原生弹窗；客户端未触发该事件或不接受决策时，插件无法强行接管 |
